...
static int32_t crc32CalByByte(uint32_t oldcrc, Byte* ptr,int offset, int len)
{
    int32_t crc = oldcrc;
    int i = offset;
    while(len-- != 0)
...
@implementation FRIBinFileCheck


+(int32_t)crc32ForBinFile:(NSData *)binData{
    
    int32_t crc32Result = 0;
    NSInteger readBuffMaxLength = 256; //max buff length 256
    
    if (binData.length > readBuffMaxLength) {
        NSInteger readLength = 0;
        NSInteger nextReadOffset = readBuffMaxLength;
        //循环调用
        do {
            readLength = (nextReadOffset+readBuffMaxLength)>binData.length?(binData.length-nextReadOffset):readBuffMaxLength;
            
            NSData *subBuffData = [binData subdataWithRange:NSMakeRange(nextReadOffset, readLength)];
            crc32Result = crc32CalByByte(crc32Result, subBuffData.bytes, 0, readLength);
    ...
    NSLog(@"===== CRC code = %@====",[NSData dataWithBytes:&crc32Result length:4]);
