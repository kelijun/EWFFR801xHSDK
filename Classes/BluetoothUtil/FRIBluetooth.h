...
@interface FRIBluetooth : NSObject
...
#pragma mark - fribluetooth filter
...
/**
friBluettooth cancelAllPeripheralsConnectionBlock 方法执行后并且全部设备断开后的回调
 ...
 *   说明：enjoy是蓝牙全套串行方法的简写（发现服务，发现特征，读取特征，读取特征描述），前面可以必须有scanForPeripherals或having方法，channel可以选择添加。
    ...
    它可以让你少敲很多代码
     ...
     ` fri.scanForPeripherals().connectToPeripherals().discoverServices().discoverCharacteristics()
     ...
     ` fri.having(self.peripheral).connectToPeripherals().discoverServices().discoverCharacteristics()
     .readValueForCharacteristic().discoverDescriptorsForCharacteristic().readValueForDescriptors().begin();
 `
 enjoy后面也可以加stop()方法
